<div class="mbo">
  ...
  <CellGroup class="bg-white mt10 setting--area" :class="{'mb50': status == 'setting'}">
    ...
    <div class="goal--box mb10 mt10">
      ...
      <div v-if="curWeek && curWeek.open_status == 2" class="setting-status color-red">{{aims_info.amount||'未设置'}}</div>
  ...
  <CellGroup v-show="status == 'review'" class="mb50">
    <div class="add--textarea--wrap">
      ...
      <textarea v-if="curWeek.open_status == 2" name="" id="" cols="3" rows="4" class="add--text--area" placeholder="开放时间已经过了，下周记得准时提交复盘总结~" disabled></textarea>
    </div>
  </CellGroup>
  ...
  <div class="button--large" v-show="status == 'review'" v-if="curWeek.open_status === 1 && aims_info.aims_replay_status == 1" @click="postReplayHandle">提交复盘总结</div>
</div>
